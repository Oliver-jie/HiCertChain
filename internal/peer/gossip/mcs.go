...
// sequence number that the block's header contains. else returns error
func (s *MSPMessageCryptoService) VerifyBlock(chainID common.ChannelID, seqNum uint64, block *pcommon.Block) error {
	...
	// 哈希方法检查验证
	if !bytes.Equal(protoutil.BlockDataHash(block.Data,block.Header.Number), block.Header.DataHash) {
