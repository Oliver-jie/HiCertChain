...
// blocksItr - an iterator for iterating over a sequence of blocks  迭代一系列块的迭代器
...
func newBlockItr(mgr *blockfileMgr, startBlockNum uint64) *blocksItr {
	mgr.blkfilesInfoCond.L.Lock()                // 加一个锁，
	defer mgr.blkfilesInfoCond.L.Unlock()        // 在函数正常返回，也就是return之后添加一个函数调用
...
// Next将光标移动到下一个块，并在迭代器未用完时返回true
