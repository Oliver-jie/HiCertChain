...
func (w *blockfileWriter) truncateFile(targetSize int) error {   // truncate 截断，缩短
...
func (w *blockfileWriter)append(b []byte, sync bool) error {
	n,_ := w.file.Seek(0,os.SEEK_END)
	_, err := w.file.WriteAt(b,n)
...
func (w *blockfileWriter)writeat(b []byte,offset int64 ,sync bool) error {
	_, err := w.file.WriteAt(b,offset)
	if err != nil {
		return err
	}
	if sync {
...
func (w *blockfileWriter) open() error {
	file, err := os.OpenFile(w.filePath, os.O_RDWR|os.O_CREATE, 0660)
	if err != nil {
