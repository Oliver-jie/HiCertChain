...
// DeliverServer is the server API for Deliver service.  传递服务的API
type DeliverServer interface {
	// Deliver first requires an Envelope of type ab.DELIVER_SEEK_INFO with   向order的数据请求的类型
	// Payload data as a marshaled orderer.SeekInfo message,                  获得区块流
	// then a stream of block replies is received
	Deliver(Deliver_DeliverServer) error
	// DeliverFiltered first requires an Envelope of type ab.DELIVER_SEEK_INFO with
	// Payload data as a marshaled orderer.SeekInfo message,                 向order的数据请求的类型
	// then a stream of **filtered** block replies is received               获得过滤器流
	DeliverFiltered(Deliver_DeliverFilteredServer) error
	// DeliverWithPrivateData first requires an Envelope of type ab.DELIVER_SEEK_INFO with
	// Payload data as a marshaled orderer.SeekInfo message,
	// then a stream of block and private data replies is received          获得区块和数据
