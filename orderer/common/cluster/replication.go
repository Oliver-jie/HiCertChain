...
func (r *Replicator) compareBootBlockWithSystemChannelLastConfigBlock(block *common.Block) {
	// Overwrite the received block's data hash（hash重新计算）
...
// Channels returns the list of ChannelGenesisBlocks for all channels.
// Each such ChannelGenesisBlock contains the genesis block of the channel.
// 此处应该和初始区块的哈希计算方式保持一致
...
func ExtractGenesisBlock(logger *flogging.FabricLogger, block *common.Block) (string, *common.Block, error) {
	...
	b := &common.Block{
		Header:   &common.BlockHeader{DataHash: protoutil.BlockDataHash(blockdata,block.Header.Number)},
		Data:     blockdata,
		Metadata: metadata,
	}
